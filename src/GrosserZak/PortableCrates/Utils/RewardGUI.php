...
class RewardGUI extends InvMenu {
    ...
    public function __construct(array $contents) {
        ...
        parent::setListener(function (InvMenuTransaction $transaction) : InvMenuTransactionResult {
            $pageNumber = $transaction->getAction()->getInventory()->getItem(49)->getNamedTag()->getCompoundTag("RebirthCrates")?->getCompoundTag("RewardGUI")?->getByte("PageNumber");
            $clickedItem = $transaction->getItemClicked();
            if(($nbt = $clickedItem->getNamedTag()->getCompoundTag("RebirthCrates")?->getCompoundTag("RewardGUI")?->getTag("GoToPage")) !== null) {
                $goTo = $nbt->getValue();
